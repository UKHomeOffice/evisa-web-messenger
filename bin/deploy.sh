...
if [[ $1 == 'tear_down' ]]; then
  ...
  $kd --delete -f kube/app -f kube/hpa.yml 
...
if [[ ${KUBE_NAMESPACE} == ${BRANCH_ENV} ]]; then
  ...
  $kd -f kube/app/deployment.yml -f kube/hpa.yml
fi

sleep $READY_FOR_TEST_DELAY

if [[ ${KUBE_NAMESPACE} == ${BRANCH_ENV} ]]; then
  echo "Branch - evisa-web-messenger-$DRONE_SOURCE_BRANCH.internal.branch.sas-notprod.homeoffice.gov.uk"
fi
